  ...
  for (i,c) in enumerate(counter):
      ...
      for(x,y) in detec:
          if y<(count_line_position+offset) and y>(count_line_position-offset):
              count=count-1
          cv2.line(frame1,(25,count_line_position),(1200,count_line_position,),(255,110,0),3)
          detec.remove((x,y))
          print("vehicle parked :" +str(count))
  if count>0:            
     cv2.putText(frame1,"car left in lot: "+str(count),(50,70),cv2.FONT_HERSHEY_SIMPLEX,2,(0,0,225),2)
  elif count==0:
     cv2.putText(frame1,"no cars left in lot",(50,70),cv2.FONT_HERSHEY_SIMPLEX,2,(0,0,225),2)
  else :
      cv2.putText(frame1,"no car left",(50,70),cv2.FONT_HERSHEY_SIMPLEX,2,(0,0,225),2)
     
  # just to show what happens in backend cv2.imshow('Detector',dilatada)
  
  cv2.imshow('PARKING OUT',frame1)
